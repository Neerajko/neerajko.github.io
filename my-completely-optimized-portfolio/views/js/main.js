...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
    ...
    // Changes the slider value to a percent width(ie,0.25 to 25)
    ...
    //document.querySelectorAll(".randomPizzaContainer") defined outside the for loop.
    var randomPizzas=document.querySelectorAll(".randomPizzaContainer");
    for(var i=0;i<randomPizzas.length;i++){
      //(var dx = determineDx(pizzaVar[0], size))=>this is really useless inside this for loop.So avoid it.
      //only percentage width is used. So avoid pixel width.
...
function updatePositions() {
  ...
  //document.body.scrollTop defined outside the loop.
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //Reduced the number of pizzas animating, bacause 200 pizzas are not necessory,only ((256/8)-1)=31 pizzas needed.
  for (var i = 0; i < 31; i++) {
